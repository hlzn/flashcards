<template>
  ...
  <br>
  <input type="checkbox" name="" id="strictModeCheckbox" v-model="strictMode" /> Strict Mode?
      ...
      <div :class="['card', { flipped: flipped[idx] }]">
        ...
        >
          <div class="front-content">
            ...
            />
            <input
              v-model="pastGuesses[idx]"
              type="text"
              placeholder="Verbo en pasado..."
              :class="{ wrong: wrong[idx] }"
            />
            <button @click.stop.prevent="check(idx)" v-if="!wrong[idx]">Comprobar</button>
            <button @click.stop.prevent="giveup(idx)" v-if="!wrong[idx]">Voltear</button>
          </div>
        </div>
        <div
...
const pastGuesses = ref<string[]>(entries.map(() => ''))
...
function check(index: number) {
  ...
  const pastGuess = pastGuesses.value[index]?.trim().toLowerCase()
  const entry = entries[index]
  const answer = entry.translation
    .trim()
    .toLowerCase()
  const pastAnswer = entry.past
    ? entry.past.trim().toLowerCase()
    : ''
...
function giveup(index: number) {
  flipped.value[index] = true
  guesses.value[index] = entries[index].translation
  pastGuesses.value[index] = entries[index].past
  wrong.value[index] = false
}
</script>
