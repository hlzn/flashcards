    ...
    <ul>
      <li v-for="(entry, index) in entries" :key="index">
        <span class="entry-row">
          <input v-model="entry.article" placeholder="articulo" />
...
function importEntries() {
  ...
  for (const item of items) {
    ...
    if (parts.length >= 3) {
      const [article, word, translation] = parts
      store.entries.push({ article, word, translation })
    }
  }
  importText.value = ''
...
function exportit() {
  const csv = entries.map(entry => {
    return `${entry.article || ''}|${entry.word}|${entry.translation}`
  }).join(', ')
